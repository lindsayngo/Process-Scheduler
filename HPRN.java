...
public class HPRN{
	...
	public static void hprn(ArrayList <Process> sortedprcs, File randomlist, Boolean v) throws FileNotFoundException {
				
			ArrayList <Process> readyprcs = new ArrayList <Process>();
				...
				readyprcs.addAll(combined);
		 		...
		 		if(processrunning == null) {
		 			...
		 			if(!readyprcs.isEmpty()) {
		 				...
		 				int index = 0;
		 				Process max = readyprcs.get(0);
		 				
		 				for(int u = 0; u < readyprcs.size(); u++) {
		 					Process current = readyprcs.get(u);
		 					if(getHPRN(cycle, max) < getHPRN(cycle, current)){
		 						max = current;
		 						index = u;
		 					}
		 					if(getHPRN(cycle, max) == getHPRN(cycle, current)) {
		 						if(max.pid > current.pid) {
		 							max = current;
		 							index = u;
		 						}
		 					}
		 		...
		 		if(processrunning != null) {
			 		...
			 		processrunning.numcyclesinrunningst += 1;
...
//				 * 5. Waiting time (time in Ready state)		
			...
			//print TOTAL summary
				...
				System.out.println("\nThe above was calculated using the scheduling method using Highest Penalty Ratio Next");
...
//				 * */	
	...
	//		System.out.println("calculated by randomOS cpubursttime: " + newint);
		...
		static double getHPRN(int cycle, Process p) {
			double numerator = cycle - p.arrivalTime;
			return  numerator / p.cpubursttime;
		}

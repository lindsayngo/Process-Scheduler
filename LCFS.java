...
public class LCFS{
	...
	public static void lcfs(ArrayList <Process> sortedprcs, File randomlist, Boolean v) throws FileNotFoundException {
			...
			ArrayList <Process> blockedprcs = new ArrayList <Process>(); //array list of blocked processes
		 	...
		 	ArrayList <Process> reverse = new ArrayList <Process>();
				...
				//check if the running process is terminated
		 		...
		 		//check if process needs to do IOtime
		 			...
		 			Process blockedprc = processrunning;
		 	 		...
		 	 		//System.out.println("process running ioTime: "+ blockedprc.ioBurst);
				...
				readyprcs.addAll(combined);
		 		...
		 		if(processrunning == null) {
		 			...
		 			if(!readyprcs.isEmpty()) {
		 				int size = readyprcs.size()-1;
			 			processrunning = readyprcs.remove(size); //remove the top and start running
...
//				 * 5. Waiting time (time in Ready state)		
			...
			//print TOTAL summary
				...
				System.out.println("\nThe above was calculated using the scheduling method using Last Come First Served");

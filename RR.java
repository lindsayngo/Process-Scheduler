...
public class RR{
	...
	public static void rr(ArrayList <Process> rrsortedprcs, File randomlist, Boolean v) throws FileNotFoundException {
			...
			Process processrunning = null;
		 	...
		 	Scanner rrscanfile = new Scanner (randomlist);
	
		 	System.out.println();
		 	
			int cycle = 0;
	 		int iousage = 0;
	 		Process preemptprc = null;
	 		int nextcpubursttime = 0;
			int extra = 0;
	 		for(int z = 0; z < rrsortedprcs.size(); z++) {
	 			allPrcs.add(rrsortedprcs.get(z));
	 		}
	...
	//	 		}
				...
				preemptprc = null; //represents the process that is preempted/put on hold because its quantum ran out
				...
				//check if the running process is terminated
		 		...
		 		//check if process needs to do IOtime
		 			...
		 			//if the process was preempted, it is now completed and 
		 			//is no longer preempted
		 			if(processrunning.preempted == true) {
		 				processrunning.preempted = false;
		 			}
		 		...
		 		else if(processrunning != null && processrunning.quantum == 0) {
		 			//if the process running still had more cpu burst time but quantum = 0
		 			//it was cut short and 
					//it must give a different process a chance to run --> make the current
					//process running to be ready
					processrunning.state = "ready";
					processrunning.preempted = true;//the process running is cut short
					preemptprc = processrunning;
					processrunning = null;
				}
		 		
		 		//if cppurburst time = 0, insert into
				...
				Iterator<Process> itrReadyPrcs = rrsortedprcs.iterator();
				...
				//add the preemptprc to the ready queue if it exists, because it is ready to run
				if(preemptprc != null) {
					arriving.add(preemptprc);
				}
				...
				if(processrunning == null && blockedprcs.isEmpty() && readyprcs.isEmpty()) {
		 		...
		 		if(processrunning == null) {
		 			...
		 			if(!readyprcs.isEmpty()) {
		 				//System.out.println("\nthere is no process running but there are ready processes");
						processrunning = readyprcs.remove();//set the first ready process as the running process
						processrunning.state = "running";
						
						if(processrunning.preempted == false) {
							//if the process was not preempted
							//calculate new cpu burst times
							int cpubursttime = randomOS(processrunning.b, rrscanfile);
							if(cpubursttime > processrunning.cpuleft) {
								cpubursttime = processrunning.cpuleft;
							}
							processrunning.cpubursttimeleft = cpubursttime;
							processrunning.cpubursttime = cpubursttime;
							processrunning.previouscpubursttime = cpubursttime;
							
						}
						//if it is preempted, it gets its old burst times
						processrunning.quantum = 2; //reset the quantum
		 		...
		 		//After all the checks: update the cpu times, cpu burst times, blocked processes etc
		 		if(processrunning != null) {
			 		...
			 		//decrease quantum by 1 each time
			 		processrunning.quantum -= 1;
			 		//System.out.println("process quantum is now " + processrunning.quantum);
		 		}
...
//				 * 5. Waiting time (time in Ready state)		
			...
			//print TOTAL summary
				...
				System.out.println("\nThe above was calculated using the scheduling method using Round Robin");
